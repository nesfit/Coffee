<template>
    <div>
        ...
        <b-modal ref="modal" hide-footer title="Product Selection" size="lg">
            <b-form>
                <TextFilter label="Display Name" v-model="displayName" />
            </b-form>

            <PagedQuery v-bind:fields="fields" endpoint="products" @item-clicked="onSelected"
    ...
    methods: {
        updateDisplay() {
            if (!this.value || !this.value.length)
                ...
                var c = this;
                State.getProductName(c.$api, c.value)
